...
class ReaderAdapter:
    ...
    def __init__(self, config: GridPorterConfig | None = None):
        """Initialize reader adapter.

        Args:
            config: GridPorter configuration
        """
        self.config = config or GridPorterConfig()
    ...
    def get_reader(self, file_path: Path, file_info: FileInfo) -> BaseReader:
        ...
        if file_info.type in {FileType.XLSX, FileType.XLS, FileType.XLSM, FileType.XLSB}:
