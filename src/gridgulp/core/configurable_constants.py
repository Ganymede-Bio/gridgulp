...
class ConfigurableConstants:
    ...
    island_detection: type[ISLAND_DETECTION.__class__]
    format_analysis: type[FORMAT_ANALYSIS.__class__]
    complex_table: type[COMPLEX_TABLE.__class__]
    cost_optimization: type[COST_OPTIMIZATION.__class__]


_default_constants = ConfigurableConstants(
    island_detection=ISLAND_DETECTION.__class__,
    format_analysis=FORMAT_ANALYSIS.__class__,
    complex_table=COMPLEX_TABLE.__class__,
    cost_optimization=COST_OPTIMIZATION.__class__,
)
...
def apply_config_overrides(config: "Config") -> None:
    ...
    if hasattr(config, "island_min_cells"):
        island_overrides["MIN_CELLS_GOOD"] = config.island_min_cells
    if hasattr(config, "island_density_threshold"):
        island_overrides["DENSITY_HIGH"] = config.island_density_threshold

    format_overrides = {}
    if hasattr(config, "format_blank_row_threshold"):
        format_overrides["BLANK_ROW_THRESHOLD"] = config.format_blank_row_threshold
    if hasattr(config, "format_total_formatting_threshold"):
        format_overrides["TOTAL_FORMATTING_THRESHOLD"] = config.format_total_formatting_threshold

    # Apply overrides if any exist
    if island_overrides:
        new_island = replace(ISLAND_DETECTION, **island_overrides)
        _current_constants = replace(_current_constants, island_detection=new_island.__class__)

    if format_overrides:
        new_format = replace(FORMAT_ANALYSIS, **format_overrides)
        _current_constants = replace(_current_constants, format_analysis=new_format.__class__)
