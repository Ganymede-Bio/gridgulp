...
async def test_format_detection():
    ...
    for file_path in test_files:
        ...
        try:
            ...
            if file_info.format_mismatch:
                ...
                print(f"   → File appears to be {file_info.type} but has .{file_path.suffix[1:]} extension")
            else:
    ...
    if results:
        print(f"{'File':<30} {'Detected':<8} {'Extension':<10} {'Match':<7} {'Confidence':<10} {'Method'}")
        print("-" * 80)

        for r in results:
            match_status = "✅" if not r['mismatch'] else "⚠️ "
            print(f"{r['file']:<30} {r['detected']:<8} {r['extension']:<10} {match_status:<7} {r['confidence']:<10.1%} {r['method']}")
