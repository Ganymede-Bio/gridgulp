...
EXPECTED_RESULTS = {
    ...
    "test_multi_sheet.xlsx": {"tables": "multiple", "description": "Multiple sheets with tables"},
    ...
    "large_table.xlsx": {"tables": 1, "description": "Large table (20 cols × 101 rows)"},
    "large_table.csv": {"tables": 1, "description": "Large table CSV"},
    "complex_table.xlsx": {"tables": 3, "description": "3 tables: Sales (A1:D6), Employees (F1:I5), Summary (A8:C10)"},
    
    # Level 2 - Complex files
    "creative_tables.xlsx": {"tables": "multiple", "description": "Creative table layouts"},
    "weird_tables.xlsx": {"tables": "multiple", "description": "Unusual table structures"},
    
    # Other
    "sample.xlsx": {"tables": "unknown", "description": "Sample file"},
    
    # Expected failures
    "corrupted.xlsx": {"tables": "error", "description": "Corrupted file (should fail)"},
    "protected.xlsx": {"tables": "error", "description": "Password protected (should fail)"},
}


async def test_file(porter: GridPorter, file_path: Path, expected: dict) -> dict:
...
async def test_all_files():
    ...
    # Initialize GridPorter with default config
    config = Config(
    ...
    for level_name, files in levels.items():
        ...
        for file_path, expected in sorted(files):
            ...
            if result["status"] == "error":
                ...
                print(f"Tables: {result['tables_found']:<2} | Time: {result['detection_time']:.3f}s | ", end="")
                if result["details"]:
                    # Show first table info
                    first_table = result["details"][0]
                    print(f"Range: {first_table['range']:<15} | Conf: {first_table['confidence']:.2%}")
                else:
            ...
            if len(result["details"]) > 1:
                for i, detail in enumerate(result["details"][1:], 2):
                    print(f"  └─ Table {i}: {detail['range']:<15} | Sheet: {detail['sheet']:<15} | Conf: {detail['confidence']:.2%}")
    ...
    if failed > 0:
        ...
        for r in all_results:
            if r["status"] == "fail":
                print(f"  - {r['file']}: Expected {r['expected']['tables']} tables, found {r['tables_found']}")
